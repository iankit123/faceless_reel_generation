...
export function Header({ hideUserMenu = false, isFixed = true }: HeaderProps) {
    ...
    return (
        <header className={`${isFixed ? 'fixed' : 'absolute'} top-0 left-0 right-0 z-50 h-16 border-b border-zinc-800 bg-zinc-950/80 backdrop-blur-md px-6 flex items-center justify-between`}>
                        ...
                        >
                            {user?.user_metadata?.avatar_url ? (
                                ...
                                <div className="w-full h-full bg-zinc-800 flex items-center justify-center">
                                    <UserIcon className={`w-5 h-5 ${user ? 'text-zinc-400' : 'text-zinc-500'}`} />
                                </div>
                            )}
                        </button>
